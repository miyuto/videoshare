...
// Bổ sung shortcodes hiển thị custom tags
function videopro_cat_tags_shortcode($atts, $content){
	$term = isset($atts['tax']) ? $atts['tax'] : 'post_tag';

	$alphas = array_merge(array('0-9'), range('A', 'Z'));
	$alphas = apply_filters('videopro_v_cats_characters', $alphas);
	
	$html = '';
	$html .= apply_filters('videopro_v_cats_before_listing', '<ul class="cat-listing ' . $term . '-listing list_tag">', $term);
	foreach($alphas as $alpha){
		...
		//	$html .= apply_filters('videopro_v_cats_heading', '<h3 class="v-cats-heading h1">' . $alpha . '</h3>', $alpha);
			 ...
			 */
			foreach($cats as $cat){
				$item = '<li><a href="' . get_term_link($cat) . '" title="' . $cat->name  . '">' . $cat->name . '</a></li>';
...
add_shortcode('v_cat_tags','videopro_cat_tags_shortcode');
