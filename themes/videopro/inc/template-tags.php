...
if(!function_exists('videopro_print_channel_social_accounts')){
	function videopro_print_channel_social_accounts($class_css = false, $id = false){
		...
		if(function_exists('videopro_get_default_social_accounts')){
			...
			$accounts = array('facebook','youtube','twitter','linkedin','tumblr','instagram','pinterest','flickr','envelope','rss');
		}
		$target ='_blank';
...
<li  class="custom-social-account <?php echo esc_attr($custom_social_account["title"]);?>"><a style="background: <?php echo esc_attr($custom_social_account["icon_background_color"]);?>" href="<?php echo esc_url($custom_social_account["url_custom_social_account"]);?>" title='<?php echo esc_attr($custom_social_account["title"]);?>' <?php echo 'target="'. esc_attr($target) . '"'; ?>><i class="<?php echo esc_attr($custom_social_account["icon_custom_social_account"]);?>"></i></a></li>
				<?php endforeach;
			endif;
			?>
		</ul>
...
if(!function_exists('videopro_print_social_accounts')){
	function videopro_print_social_accounts(){
		/* below are default supported social networks. To support more, add the name of theme option in the array */
		$accounts = array('facebook','youtube','twitter','linkedin','tumblr','instagram','pinterest','flickr','envelope','rss','twitch');
...
if(!function_exists('videopro_breadcrumbs')){
	function videopro_breadcrumbs($echo = true, $class = ''){
		...
		if(ot_get_option('enable_breadcrumbs','on') == 'off'){
			if($auto_next_html != ''){
				echo '<ol class="cactus-breadcrumb ' . esc_attr($class) . ' navigation-font font-size-1 '. esc_attr($at_class) . '"><span>&nbsp;</span>';
				echo $auto_next_html;
				echo '</ol>';
			}
		...
		if(is_active_sidebar('breadcrumbs-sidebar')){
			echo '<ol class="cactus-breadcrumb ' . esc_attr($class) . ' navigation-font font-size-1 '. esc_attr($at_class) .'">';
		...
		$after          = $auto_next_html; // tag after the current crumb
		/* === END OF OPTIONS === */
		...
		if(is_front_page()) {
			...
			if ($show_on_home == 1) echo '<ol class="cactus-breadcrumb ' . esc_attr($class) . ' navigation-font font-size-1 '. esc_attr($at_class) . '" itemscope itemtype="http://schema.org/BreadcrumbList"><a itemprop="item" href="' . esc_url($home_link) . '"><span itemprop="name">' . $text['home'] .'</span></a>'. sprintf($meta, $position) .'</li></ol>';

		} elseif(is_home()) {

			$title = get_option('page_for_posts') ? get_the_title(get_option('page_for_posts')) : esc_html__('Blog','videopro');
			echo '<ol class="cactus-breadcrumb ' . esc_attr($class) . ' navigation-font font-size-1 '. esc_attr($at_class) . '" itemscope itemtype="http://schema.org/BreadcrumbList"><a href="' . esc_url($home_link) . '"><span itemprop="name">' . $text['home'] . '</span></a>'.sprintf($meta, $position). ' \ '.$title.'</li></ol>';

		} elseif (function_exists("is_woocommerce") && is_woocommerce() && is_product()) { // consider later
			$args = array(
				'delimiter' => '<i class="fas fa-angle-right" aria-hidden="true"></i>',
				'home' => esc_html__('Home', 'videopro')
			);
			echo '<ol class="cactus-breadcrumb ' . esc_attr($class) . ' navigation-font font-size-1 ' . esc_attr($at_class) . '" itemscope itemtype="http://schema.org/BreadcrumbList">';
			...
			if ($show_home_link == 1) {
				if(function_exists ( "is_shop" ) && is_shop()){
					...
					echo '<a itemprop="item" href="' . esc_url($home_link) . '" rel="v:url" property="v:title">' . '<span itemprop="name">' .$text['home'] . '</span></a>';
					if ($frontpage_id == 0 || $parent_id != $frontpage_id) echo $delimiter;
				}
			}

			if ( is_search() ) {
				echo $before . sprintf($text['search'], get_search_query()) . '</span>' . sprintf($meta, ++$position) . '</li>'. $after;
				...
				if ($show_current == 1) echo $before . sprintf($text['category'], single_cat_title('', false)) . '</span>' . sprintf($meta, ++$position) . '</li>'. $after;

			} elseif ( is_day() ) {
				echo sprintf($link, get_year_link(get_the_time('Y')), get_the_time('Y')) . $delimiter;
				echo sprintf($link, get_month_link(get_the_time('Y'),get_the_time('m')), get_the_time('F')) . $delimiter;
				echo $before . get_the_time('d'). '</span>' . sprintf($meta, ++$position) . '</li>' . $after;

			} elseif ( is_month() ) {
				echo sprintf($link, get_year_link(get_the_time('Y')), get_the_time('Y')) . $delimiter;
				echo $before . get_the_time('F') . '</span>' . sprintf($meta, ++$position) . '</li>'. $after;

			} elseif ( is_year() ) {
				echo $before . get_the_time('Y') . '</span>' . sprintf($meta, ++$position) . '</li>'. $after;
				...
				if ( get_post_type() != 'post' ) {
					...
					if ($show_current == 1) echo $delimiter . $before . $post->post_title . '</span>' . sprintf($meta, ++$position) . $after;
				} else {
					...
					if ($show_current == 1) echo $before . $post->post_title . '</span>' . sprintf($meta, ++$position) . '</li>'. $after;
				}
				...
				if(function_exists ( "is_shop" ) && is_shop()){
					...
					$post_type = get_post_type_object(get_post_type());
					$slug = $post_type->rewrite;
					echo $before . ($slug['slug'] ? $slug['slug'] : $post_type->labels->singular_name) . '</span>' . sprintf($meta, ++$position) . '</li>'. $after;
				...
				if ($show_current == 1) echo $delimiter . $before . get_the_title() . '</span>' . sprintf($meta, ++$position) . '</li>'. $after;

			} elseif ( is_page() && !$parent_id ) {
				if ($show_current == 1) echo $before . get_the_title() . '</span>' . sprintf($meta, ++$position) . '</li>'. $after;
				...
				if ($show_current == 1) {
					...
					echo $before . get_the_title() . '</span>' . sprintf($meta, ++$position) . '</li>'. $after;
				}

			} elseif ( is_tag() ) {
				echo $before . sprintf($text['tag'], single_tag_title('', false)) . '</span>' . sprintf($meta, ++$position) . '</li>'. $after;

			} elseif ( is_author() ) {
				global $author;
				$userdata = get_userdata($author);
				echo $before . sprintf($text['author'], $userdata->display_name) . '</span>' . sprintf($meta, ++$position) . '</li>'. $after;

			} elseif ( is_404() ) {
				echo $before . $text['404'] . '</span>' . sprintf($meta, ++$position) . '</li>'. $after;
			}
